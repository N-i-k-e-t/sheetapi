...
const Dashboard: React.FC = () => {
  ...
  const [apiLogs, setApiLogs] = useState<ApiCallLog[]>(db.getApiLogs());
    ...
    try {
      ...
      workbook.SheetNames.forEach(name => {
        ...
        json.forEach((row: any) => {
          db.saveRecord({ date: ts, raw_data: row });
          count++;
        });
      });
  ...
  const handleManualAdd = (e: React.FormEvent) => {
    ...
    try {
      ...
      db.saveRecord({ date: new Date().toISOString(), raw_data: parsed });
      setManualRow('');
      refreshState();
  ...
  const simulateApiCall = () => {
    db.logApiCall('Internal Test Client');
    refreshState();
  };
      ...
      <div className="flex overflow-x-auto border-b border-slate-100 mb-8 pb-px space-x-10 no-scrollbar">
        {[
          ...
          <button
            ...
            className={`pb-4 text-[10px] font-black tracking-[0.2em] uppercase flex items-center space-x-2 transition-all border-b-2 whitespace-nowrap ${
