...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!db.getApiKey()) db.generateApiKey();
    ...
    const triggerAutoPull = async () => {
      ...
      try {
        ...
        db.createBackup('Scheduled Auto-Sync Backup');

        workbook.SheetNames.forEach(name => {
          const rows = XLSX.utils.sheet_to_json(workbook.Sheets[name]);
          rows.forEach((r: any) => {
            db.saveRecord({ date: timestamp, raw_data: r });
            rowCount++;
          });
        });

        db.saveSettings({ ...settings, lastSync: timestamp, lastScheduledSyncDate: todayStr });
        db.addSyncLog({ status: 'SUCCESS', message: `Daily Auto-Pull: ${rowCount} rows added.` });
      } catch (e) {
